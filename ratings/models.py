...
# --- Flag (Represents Descriptors - No changes) ---
...
class Game(models.Model):
    ...
    distorting_islam_severity = models.CharField(_('A1. Distorting Islam Severity'), max_length=1, choices=SEVERITY_CHOICES, default='N')
    promoting_kufr_severity = models.CharField(_('A2. Promoting Kufr/Shirk Severity'), max_length=1, choices=SEVERITY_CHOICES, default='N')
    assuming_divinity_severity = models.CharField(_('A3. Assuming Divinity Severity'), max_length=1, choices=SEVERITY_CHOICES, default='N')
    tampering_ghaib_severity = models.CharField(_('A4. Tampering Ghaib Severity'), max_length=1, choices=SEVERITY_CHOICES, default='N')
    deviant_ideologies_severity = models.CharField(_('A5. Deviant Ideologies Severity'), max_length=1, choices=SEVERITY_CHOICES, default='N')
    gambling_severity = models.CharField(_('B1. Gambling Severity'), max_length=1, choices=SEVERITY_CHOICES, default='N')
    lying_severity = models.CharField(_('B2. Lying Severity'), max_length=1, choices=SEVERITY_CHOICES, default='N')
    indecency_severity = models.CharField(_('B3. Indecency Severity'), max_length=1, choices=SEVERITY_CHOICES, default='N')
    music_instruments_severity = models.CharField(_('B4. Music/Instruments Severity'), max_length=1, choices=SEVERITY_CHOICES, default='N')
    time_waste_severity = models.CharField(_('B5. Time Waste Severity'), max_length=1, choices=SEVERITY_CHOICES, default='N')
    disdain_arrogance_severity = models.CharField(_('C1. Normalizing Disdain/Arrogance Severity'), max_length=1, choices=SEVERITY_CHOICES, default='N')
    magic_severity = models.CharField(_('C2. Normalizing Magic Severity'), max_length=1, choices=SEVERITY_CHOICES, default='N')
    intoxicants_severity = models.CharField(_('C3. Normalizing Intoxicants Severity'), max_length=1, choices=SEVERITY_CHOICES, default='N')
    crime_violence_severity = models.CharField(_('C4. Normalizing Crime/Violence Severity'), max_length=1, choices=SEVERITY_CHOICES, default='N')
    profanity_severity = models.CharField(_('C5. Normalizing Profanity Severity'), max_length=1, choices=SEVERITY_CHOICES, default='N')
    horror_fear_severity = models.CharField(_('D1. Horror/Fear Severity'), max_length=1, choices=SEVERITY_CHOICES, default='N')
    despair_severity = models.CharField(_('D2. Despair Severity'), max_length=1, choices=SEVERITY_CHOICES, default='N')
    spending_severity = models.CharField(_('D3. Spending Severity'), max_length=1, choices=SEVERITY_CHOICES, default='N')
    online_interactions_severity = models.CharField(_('D4. Online Interactions Severity'), max_length=1, choices=SEVERITY_CHOICES, default='N')
    user_content_severity = models.CharField(_('D5. User Content Severity'), max_length=1, choices=SEVERITY_CHOICES, default='N')
    ...
    SEVERITY_FIELD_TO_FLAG_SYMBOL = { 'distorting_islam_severity': 'history_edu', 'promoting_kufr_severity': 'hub', 'assuming_divinity_severity': 'stars', 'tampering_ghaib_severity': 'visibility', 'deviant_ideologies_severity': 'balance', 'gambling_severity': 'casino', 'lying_severity': 'front_hand', 'indecency_severity': 'visibility_off', 'music_instruments_severity': 'music_off', 'time_waste_severity': 'hourglass_top', 'disdain_arrogance_severity': 'sentiment_dissatisfied', 'magic_severity': 'auto_fix_high', 'intoxicants_severity': 'local_bar', 'crime_violence_severity': 'local_police', 'profanity_severity': 'speaker_notes_off', 'horror_fear_severity': 'report', 'despair_severity': 'sentiment_sad', 'spending_severity': 'monetization_on', 'online_interactions_severity': 'forum', 'user_content_severity': 'extension', }
    CATEGORY_A_FIELDS = [ 'distorting_islam_severity', 'promoting_kufr_severity', 'assuming_divinity_severity', 'tampering_ghaib_severity', 'deviant_ideologies_severity']
    CATEGORY_B_FIELDS = [ 'gambling_severity', 'lying_severity', 'indecency_severity', 'music_instruments_severity', 'time_waste_severity']
    CATEGORY_C_FIELDS = [ 'disdain_arrogance_severity', 'magic_severity', 'intoxicants_severity', 'crime_violence_severity', 'profanity_severity']
    CATEGORY_D_FIELDS = [ 'horror_fear_severity', 'despair_severity', 'spending_severity', 'online_interactions_severity', 'user_content_severity']
    ALL_DESCRIPTOR_FIELDS_IN_ORDER = CATEGORY_A_FIELDS + CATEGORY_B_FIELDS + CATEGORY_C_FIELDS + CATEGORY_D_FIELDS

    class Meta: ordering = ['-date_updated']; verbose_name = _('Game'); verbose_name_plural = _('Games')
    def _calculate_rating_tier(self):
        ...
        except RatingTier.DoesNotExist: print("ERROR: Rating Tiers missing for calculation."); return RatingTier.objects.filter(tier_code='MSH').first()
        if any(getattr(self, f, 'N') == 'S' for f in self.CATEGORY_A_FIELDS): return tier_kfr
        if any(getattr(self, f, 'N') == 'M' for f in self.CATEGORY_A_FIELDS) or \
           any(getattr(self, f, 'N') == 'S' for f in self.CATEGORY_B_FIELDS): return tier_hrm
        if any(getattr(self, f, 'N') == 'L' for f in self.CATEGORY_A_FIELDS) or \
           any(getattr(self, f, 'N') == 'M' for f in self.CATEGORY_B_FIELDS) or \
           any(getattr(self, f, 'N') in ['M', 'S'] for f in self.CATEGORY_C_FIELDS) or \
           any(getattr(self, f, 'N') == 'S' for f in self.CATEGORY_D_FIELDS): return tier_msh
        return tier_hal
    def _get_flags_to_set(self):
        flag_symbols = {self.SEVERITY_FIELD_TO_FLAG_SYMBOL[f] for f in self.ALL_DESCRIPTOR_FIELDS_IN_ORDER if getattr(self, f, 'N') != 'N' and f in self.SEVERITY_FIELD_TO_FLAG_SYMBOL}
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.pk or Game.objects.filter(slug=self.slug).exclude(pk=self.pk).exists():
            ...
            while Game.objects.filter(slug=self.slug).exclude(pk=self.pk).exists(): self.slug = f'{original_slug}-{counter}'; counter += 1
        ...
        if calculated_tier: self.rating_tier = calculated_tier
             ...
             try: self.rating_tier = RatingTier.objects.get(tier_code='MSH')
             except RatingTier.DoesNotExist: print(f"ERROR: Cannot set default MSH tier for game '{self.title}'.")
        super().save(*args, **kwargs)
        if self.pk:
            try:
                flags_to_set = self._get_flags_to_set()
                current_flags = set(self.flags.all())
                if set(flags_to_set) != current_flags: self.flags.set(flags_to_set)
            except Exception as e: print(f"Error setting flags for game {self.pk} ('{self.title}'): {e}")
    def get_absolute_url(self): return reverse('ratings:game_detail', kwargs={'game_slug': self.slug})
    def __str__(self): return self.title
    def get_severity_display_name(self, severity_code): return dict(self.SEVERITY_CHOICES).get(severity_code, '?')
...
class GameComment(models.Model):
    ...
    # CORRECTED __str__ method with proper indentation
    def __str__(self):
...
# --- MethodologyPage & WhyMGCPage (No changes) ---
class MethodologyPage(models.Model):
    title = models.CharField(max_length=200, verbose_name=_('Page Title')); content = RichTextUploadingField(verbose_name=_('Page Content')); last_updated = models.DateTimeField(auto_now=True, verbose_name=_('Last Updated'))
    class Meta: verbose_name=_("Methodology Page"); verbose_name_plural=_("Methodology Page")
    def __str__(self): return self.title

class WhyMGCPage(models.Model):
    title = models.CharField(max_length=200, default=_('Why MGC?'), verbose_name=_('Page Title')); content = RichTextUploadingField(verbose_name=_('Page Content')); last_updated = models.DateTimeField(auto_now=True, verbose_name=_('Last Updated'))
    class Meta: verbose_name=_("Why MGC Page"); verbose_name_plural=_("Why MGC Page")
    def __str__(self): return str(self.title)

# --- Suggestion Model ---
class Suggestion(models.Model):
    SUGGESTION_TYPES = [ ('NEW_DESCRIPTOR', _('Suggest New Descriptor')), ('CHANGE_SEVERITY', _('Suggest Severity Change')), ('OTHER', _('Other Feedback/Correction')), ]
    SUGGESTION_STATUS = [ ('PENDING', _('Pending Review')), ('APPROVED', _('Approved')), ('IMPLEMENTED', _('Implemented')), ('REJECTED', _('Rejected')), ]
    ...
    class Meta: ordering=['-created_date']; verbose_name=_('User Suggestion'); verbose_name_plural=_('User Suggestions')
    def __str__(self):
        user_str = self.user.username if self.user else _('Anonymous')
        if self.suggestion_type == 'CHANGE_SEVERITY' and self.game and self.existing_descriptor: return _("Severity suggestion for '%(descriptor)s' on '%(game)s' by %(user)s") % {'descriptor': self.existing_descriptor.description, 'game': self.game.title, 'user': user_str}
        elif self.suggestion_type == 'NEW_DESCRIPTOR' and self.suggested_descriptor_name: return _("New descriptor suggestion '%(name)s' by %(user)s") % {'name': self.suggested_descriptor_name, 'user': user_str}
        else: return _("Suggestion by %(user)s (%(type)s)") % {'user': user_str, 'type': self.get_suggestion_type_display()}
    def clean(self):
        super().clean()
        if self.suggestion_type == 'CHANGE_SEVERITY':
            if not self.game or not self.existing_descriptor or not self.suggested_severity: raise ValidationError(_("For severity change suggestions, please specify the game, the descriptor, and the suggested severity."))
            if self.game and self.existing_descriptor:
                 ...
                 if field_name: self.current_severity = getattr(self.game, field_name, 'N')
        elif self.suggestion_type == 'NEW_DESCRIPTOR':
            if not self.suggested_descriptor_name: raise ValidationError(_("For new descriptor suggestions, please provide a name."))
